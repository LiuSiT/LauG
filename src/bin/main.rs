    ...
    while let Some(e) = events.next(&mut window) {
        ...
        if let Some(button) = e.release_args() {
            match button {
                ...
                Button::Keyboard(key) => button_key_move(key, &mut person_cursor),
...
fn button_key_move(key: piston::Key, cursor: &mut [f64; 2]){
    if key == piston::Key::W {
        println!("方向向上");
        cursor[1] -= 10.0;
    }else if key == piston::Key::A {
        println!("方向向左");
        cursor[0] -= 10.0;
    }else if key == piston::Key::S {
        println!("方向向下");
        cursor[1] += 10.0;
    }else if key == piston::Key::D {
        println!("方向向右");
        cursor[0] += 10.0;
    }
